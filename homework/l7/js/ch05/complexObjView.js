...
function handleKeyDown(event) {
    ...
    if( changePos === 1 ){
        switch (key) {
            case 65: //键盘对应键：left、a
                dx -= step;
                document.getElementById("xpos").value=dx;
                break;
            case 68: //键盘对应键：right、d
                dx += step;
                document.getElementById("xpos").value=dx;
                break;
            case 87: //键盘对应键：up、w
...
function checkInput(){
    var ptype = document.getElementById( "ortho" ).checked;//正交模式
    if( ptype ) {
        projectionType = 1;
    }else{
        if( document.getElementById( "persp" ).checked )//投影模式
            projectionType = 2;
    }
